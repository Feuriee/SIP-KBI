...
<body class="bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition duration-500">
            ...
            <div class="p-6">
                ...
                <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <!-- Search Input -->
                        <input type="text" id="search-input" placeholder="Cari nama pakan atau nama kolam..."
    ...
    <script>
        ...
        // Load Jadwal dengan filter dan search
        async function loadJadwal(search = '', beratFilter = '') {
            try {
                const params = new URLSearchParams();
                if (search) params.append('search', search);

                // Filter dan sort berdasarkan berat (jumlah_kg)
                if (beratFilter === 'terberat') {
                    params.append('sort_by', 'jumlah_kg');
                    params.append('sort_order', 'desc');
                } else if (beratFilter === 'ringan') {
                    params.append('sort_by', 'jumlah_kg');
                    params.append('sort_order', 'asc');
                }

                const queryString = params.toString();
                const url = `/api/jadwal-pakan${queryString ? '?' + queryString : ''}`;
        ...
        function applyFilters() {
            ...
            const beratFilter = document.getElementById('status-filter').value;

            loadJadwal(search, beratFilter);
        }
        ...
        function onBeratFilterChange() {
            // Apply filter langsung saat berat berubah
            applyFilters();
        }
        ...
        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
            ...
            <td class="px-6 py-4 text-center">
                <div class="flex justify-center space-x-2">
                    <button onclick='editJadwal(${JSON.stringify(item)})' class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300" title="Edit">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                        </svg>
                    </button>
                    <button onclick="deleteJadwal(${item.id})" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300" title="Hapus">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
