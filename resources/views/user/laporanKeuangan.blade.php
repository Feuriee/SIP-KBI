...
<body class="bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition duration-500">
    ...
    <div class="flex min-h-screen">
            ...
            <div class="p-6">
                ...
                <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
                    ...
                    bg-white dark:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-sipkbi-green">
                        ...
                        <!-- Filter Laba -->
                        <select id="laba-filter"
                            class="border border-gray-300 dark:border-gray-600 rounded-lg px-3 h-10
                    bg-white dark:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-sipkbi-green">
                            <option value="">Semua Status</option>
                            <option value="tertinggi">Laba Tertinggi</option>
                            <option value="terendah">Laba Terendah</option>
                            <option value="untung">Untung Saja</option>
                            <option value="rugi">Rugi Saja</option>
                        </select>
                        ...
                        <table class="w-full">
                            <thead class="bg-sipkbi-green text-white">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase">Total Pendapatan
                                    </th>
                                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase">Total Pengeluaran
                                    </th>
                        ...
                        <form id="laporan-form" class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" step="0.01" id="total_pendapatan" required
                                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-2 focus:ring-sipkbi-green focus:border-transparent">
                                </div>

                                <div>
                                    <label class="block text-sm font-medium mb-2">Total Pengeluaran (Rp)</label>
                                    <input type="number" step="0.01" id="total_pengeluaran" required
                                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-2 focus:ring-sipkbi-green focus:border-transparent">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" id="laba_bersih" readonly
        ...
        <script>
            ...
            // Load Laporan dengan filter dan search
            async function loadLaporan(search = '', bulan = '', labaFilter = '') {
                try {
                    ...
                    // Filter laba (untung/rugi/tertinggi/terendah)
                    if (labaFilter === 'tertinggi') {
                        params.append('filter_laba', 'tertinggi');
                    } else if (labaFilter === 'terendah') {
                        params.append('filter_laba', 'terendah');
                    } else if (labaFilter === 'untung') {
                        params.append('filter_laba', 'untung');
                    } else if (labaFilter === 'rugi') {
                        params.append('filter_laba', 'rugi');
                    }

                    const queryString = params.toString();
                    const url = `/api/laporan-keuangan${queryString ? '?' + queryString : ''}`;
            ...
            function exportToExcel() {
                try {
                    ...
                    const excelData = allData.map((item, index) => {
                        ...
                        const status = labaBersih >= 0 ? 'Untung' : 'Rugi';

                        return {
                            'No': index + 1,
                            'Bulan': bulanDate.toLocaleDateString('id-ID', { year: 'numeric', month: 'long' }),
                            'Total Pendapatan': parseFloat(item.total_pendapatan),
                            'Total Pengeluaran': parseFloat(item.total_pengeluaran),
                            'Laba Bersih': Math.abs(labaBersih),
                            'Status': status,
                    ...
                    const date = new Date();
                    const filename = `Laporan_Keuangan_${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}.xlsx`;
        ...
        function changePage(page) {
            ...
            function applyFilters() {
                ...
                const labaFilter = document.getElementById('laba-filter').value;

                loadLaporan(search, bulan, labaFilter);
            }
            ...
            function resetFilters() {
                ...
                document.getElementById('laba-filter').value = '';
                loadLaporan();
            }
            ...
            document.getElementById('laba-filter').addEventListener('change', onFilterChange);
        ...
        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
            ...
            <td class="px-6 py-4 text-center">
                <div class="flex justify-center space-x-2">
                    <button onclick='editLaporan(${JSON.stringify(item)})' class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300" title="Edit">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                        </svg>
                    </button>
                    <button onclick="deleteLaporan(${item.id})" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300" title="Hapus">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
