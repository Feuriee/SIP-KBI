    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 lg:ml-64">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 mb-6">
                    <div class="flex flex-col md:flex-row gap-4">
                        ...
                        <button onclick="applyFilters()"
                            class="bg-sipkbi-green hover:bg-sipkbi-dark text-white px-6 py-2 rounded-lg flex items-center justify-center gap-2 transition">
                        ...
                        <button onclick="resetFilters()"
                            class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg flex items-center justify-center gap-2 transition">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M6 18L18 6M6 6l12 12"></path>
                            </svg>
                    ...
                    <!-- Filter Info -->
                    <div id="filter-info" class="mt-3 text-sm text-gray-600 dark:text-gray-400 hidden">
                            ...
                            <form id="user-form" class="space-y-4" novalidate>
                                ...
                                <div>
                                    ...
                                    <input type="password" id="password"
                                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-2 focus:ring-sipkbi-green focus:border-transparent">
                                    <p class="text-xs text-gray-500 mt-1">Kosongkan jika tidak ingin mengubah password
                                    </p>
                                </div>
            ...
            <script>
                ...
                // Load Data
                async function loadData(search = '', role = '') {
                    try {
                        ...
                        renderTable(data);
                        updateFilterInfo(search, role, data.length);
                    } catch (error) {
        ...
        `;
                ...
                // Update Filter Info
                function updateFilterInfo(search, role, count) {
                    const filterInfo = document.getElementById('filter-info');
                    const filterDetails = document.getElementById('filter-details');

                    let details = [];
                    if (search) details.push(`Pencarian: "${search}"`);
                    if (role) details.push(`Role: ${role === 'admin' ? 'Admin' : 'User'}`);

                    if (details.length > 0) {
                        filterDetails.textContent = details.join(', ') + ` - ${count} data ditemukan`;
                ...
                userForm.addEventListener('submit', async (e) => {
                    ...
                    try {
                        ...
                        closeModal();
                        applyFilters(); // Reload dengan filter yang aktif
                    } catch (error) {
                        console.error('Error:', error);
